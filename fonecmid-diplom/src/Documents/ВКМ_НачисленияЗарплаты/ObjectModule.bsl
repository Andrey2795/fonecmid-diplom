...
Процедура СформироватьДвижения();
	...
	Для Каждого Строка Из Начисления Цикл
		...
		Если ТипЗнч(Строка.ВидРасчета) = Тип ("ПланВидовРасчетаСсылка.ВКМ_ОсновныеНачисления") Тогда
			...
			Движение.Размер = РазмерОклада(Строка.Сотрудник);
	...
	Пока Выборка.Следующий() Цикл
		...
		Если Движение.ВидРасчета.Наименование = "Оклад" Тогда
			Если Выборка.Сотрудник.ВКМ_Должность = Перечисления.ВКМ_Должности.Специалист Тогда
				...
				Если СуммаПроцентОтРабот <> Неопределено Тогда
					Движение.Размер = Окр((РазмерОклада(Выборка.Сотрудник) / Выборка.РабочихДней) * Выборка.ОтработаноДней, 2) + СуммаПроцентОтРабот;
				Иначе
					Движение.Размер = Окр((РазмерОклада(Выборка.Сотрудник) / Выборка.РабочихДней) * Выборка.ОтработаноДней, 2);
				КонецЕсли;
			КонецЕсли;	
		Иначе
			//@skip-check query-in-loop
			НачисленнаяСуммаЗаГод = НачисленнаяСуммаЗаГод(Выборка.Сотрудник);
			Если НачисленнаяСуммаЗаГод <> Неопределено Тогда
...
Процедура РассчитатьУдержания();
	...
	|	СУММА(ВКМ_ОсновныеНачисления.Размер) КАК Размер
	|ИЗ
	...
	|	СУММА(ВКМ_ДополнительныеНачисления.Размер)
	|ИЗ
