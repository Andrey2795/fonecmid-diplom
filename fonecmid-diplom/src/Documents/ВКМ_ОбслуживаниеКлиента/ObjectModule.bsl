...
Процедура ОбработкаПроведения(Отказ,Режим)
	...
	ПроцентОтОплатыРабот = ПроцентОтОплатыСотрудника();
	
	Если ПроцентОтОплатыРабот <> Неопределено Тогда
		...
		Для Каждого ТекСтрокаВыполненныеРаботы из ВыполненныеРаботы Цикл
			...
			Движение.СуммаКОплате = ТекСтрокаВыполненныеРаботы.ЧасыКОплатеКлиенту * Договор.СтоимостьЧасаРабот * ПроцентОтОплатыРабот / 100;
		КонецЦикла;		
	Иначе
		Отказ = Истина;
	КонецЕсли;	
